"""
TIC-VLA VLM-Only Model for training just the Vision-Language Model component.
...
class TICVLA_VLM(nn.Module):
    ...
    def __init__(self, model_path: str = 'InternVL3-1B', device: str = 'cuda:0') -> None:
        """
        ...
        # Freeze the vision backbone (optional - can be unfrozen for fine-tuning)
    ...
    def _extract_waypoint_from_response(self, response: str) -> Optional[torch.Tensor]:
        ...
        print(f"[TICVLA_VLM] Full VLM response:\n{response}\n[TICVLA_VLM] End of response")
        print(f"[TICVLA_VLM] Response length: {len(response)} characters")
...
if __name__ == "__main__":
    # Test the model
    path = 'InternVL3-1B'
    model = TICVLA_VLM(model_path=path, device='cuda:0')
